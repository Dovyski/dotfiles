...
function Configure-TerminalAliases {
    $aliasesUrl = "https://raw.githubusercontent.com/Dovyski/dotfiles/main/aliases.ps1"

    Write-Host "Fetching aliases from: $aliasesUrl"
    $block = (Invoke-WebRequest -Uri $aliasesUrl -UseBasicParsing).Content

    # Load aliases in current session
    Write-Host "Loading aliases in current session..."
